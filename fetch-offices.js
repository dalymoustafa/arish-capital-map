...
function extractCity(address) {
  ...
  for (let i = parts.length - 2; i >= 0; i--) {
    ...
    if (/^\d+$/.test(part)) continue;
    if (/\b(street|road|avenue|boulevard|floor|level|suite|tower|plaza|bldg|place|utca|district|avenida|rua|heights|zayed)\b/i.test(part)) continue;
...
async function main() {
  ...
  const record = allRecords.find(r =>
    r.fields["Name"] && r.fields["Name"].toLowerCase().includes("arish capital")
  );

  if (!record) { console.error("Could not find Arish Capital Partners!"); process.exit(1); }
...
<head>
  ...
  <title>Arish Capital Partners — Office Locations</title>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    #imi-map { width: 100%; height: 280px; background: #ffffff; }
    .leaflet-control-attribution { display: none !important; }
    .leaflet-control-zoom { display: none !important; }
    .custom-tooltip { position: absolute; z-index: 9999; background: #fff; border: 1px solid #e4e4e4; box-shadow: 0 4px 16px rgba(0,0,0,0.10); padding: 10px 12px; width: 180px; display: none; pointer-events: none; }
